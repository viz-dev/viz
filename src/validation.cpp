...
#if defined(NDEBUG)
# error "Viz cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Viz Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s VIZ additional fees, %d delta bytes\n",
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Viz: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
...
{
    LOCK(cs_main);
    LogPrintf("VersionBitsTipState calling VersionBitsState()\n");
